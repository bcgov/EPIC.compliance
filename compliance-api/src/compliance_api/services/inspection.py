    ...
    def get_inspection_details(cls, case_file_id):
        ...
        for inspection in inspections:
            ...
            requirement_details = _make_requirement_detail_object_optimized(
                requirements, enforcement_actions_data.get(inspection.id, {})
            )
            ...
            if action_type == EnforcementActionOptionEnum.ORDER:
                ...
                item["enforcement_action"] = (
                    _set_violation_ticket_enforcement_action_object(
                        item["enforcement_action"], violation_tickets, requirement
                    )
                )
            elif (
                action_type
...
def _set_restorative_justice_enforcement_action_object(
    ...
    if requirement_restorative_justice:
        rj = requirement_restorative_justice[0]
        enforcement_action["status"] = (
            {
                "id": rj.status.name,
                "name": rj.status.value,
            }
            if rj.status
            else None
        )
        enforcement_action["number"] = rj.restorative_justice_number
...
def _bulk_fetch_enforcement_actions(inspection_ids):
    ...
    if inspection_ids:
        ...
        for wl in warning_letters:
            enforcement_data[wl.inspection_id]["warning_letters"].append(wl)
        ...
        for vt in violation_tickets:
            enforcement_data[vt.inspection_id]["violation_tickets"].append(vt)
        ...
        for ap in admin_penalties:
            enforcement_data[ap.inspection_id]["administrative_penalties"].append(ap)
        ...
        for cr in charge_recommendations:
            enforcement_data[cr.inspection_id]["charge_recommendations"].append(cr)
        ...
        for rj in restorative_justice:
            enforcement_data[rj.inspection_id]["restorative_justice"].append(rj)
