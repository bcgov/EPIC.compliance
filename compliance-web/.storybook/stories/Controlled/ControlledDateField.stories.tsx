...
const meta: Meta<typeof ControlledDateField> = {
  title: 'Components/Controlled/ControlledDateField',
...
export const WithPreSelectedDate: Story = {
  decorators: [
    (Story) => (
      <FormDecorator
        defaultFormValues={{ dateField: new Date('2024-01-15') }}
      >
        <Story />
      </FormDecorator>
    ),
...
export const WithValidation: Story = {
  decorators: [
    (Story) => (
      <FormDecorator
        schema={yup.object({
          dateField: yup.date().required('Date is required').min(new Date(), 'Date must be in the future'),
        })}
